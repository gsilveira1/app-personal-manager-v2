...
import { PaymentStatus, PaymentMethod, type FinanceRecord } from '../types'
import { Card, Badge, Button, Input, Select, Label } from '../components/ui'
import { Filter, Search, PlusCircle, CheckCircle2, X, Wallet } from 'lucide-react'
import { format, parseISO, startOfDay, endOfDay, subMonths, isSameMonth } from 'date-fns'
import { useSearchParams } from 'react-router'
import { Link } from 'react-router-dom'
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart, Pie, Cell, Legend } from 'recharts'

export const Finances = () => {
  const { finances, clients, products, generateMonthlyInvoices, markFinanceRecordPaid, addFinanceRecord } = useStore()
  ...
  const [chartViewDate] = useState(new Date())
  ...
  const filteredFinances = useMemo(() => {
    return finances
      .filter((record) => {
        ...
        // Filter Logic
        const matchesSearch = filters.search === '' || clientName.includes(filters.search.toLowerCase()) || record.description.toLowerCase().includes(filters.search.toLowerCase())
  ...
  const revenueTrendData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthTotal = finances.filter((f) => isSameMonth(parseISO(f.date), d) && f.status === PaymentStatus.Paid).reduce((acc, curr) => acc + curr.amount, 0)
  ...
  const compositionData = useMemo(() => {
    const subscriptions = finances.filter((f) => f.type === 'Subscription' && f.status === PaymentStatus.Paid).reduce((acc, curr) => acc + curr.amount, 0)
    const products = finances.filter((f) => f.type === 'OneTime' && f.status === PaymentStatus.Paid).reduce((acc, curr) => acc + curr.amount, 0)
        ...
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 w-full xl:w-auto">
            ...
            <div className="relative flex-1 sm:w-64">
              ...
              <Input placeholder="Search..." className="pl-9" value={filters.search} onChange={(e) => setFilters((prev) => ({ ...prev, search: e.target.value }))} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  setFilters({ status: 'All', method: 'All', search: '', clientId: 'All' })
                }}
        ...
        <div className="p-6 border-b border-slate-200 flex justify-between items-center">
          ...
          {filters.clientId !== 'All' && (
            <Badge variant="default" className="flex items-center gap-1">
              Filtered by Client{' '}
              <button onClick={() => setFilters((prev) => ({ ...prev, clientId: 'All' }))}>
              ...
              {filteredFinances.map((record) => {
                ...
                return (
                  <tr key={record.id} className="hover:bg-slate-50">
                    <td className="px-6 py-4 text-slate-600 whitespace-nowrap">{format(parseISO(record.date), 'MMM d, yyyy')}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge variant={record.status === PaymentStatus.Paid ? 'success' : record.status === PaymentStatus.Pending ? 'warning' : 'error'}>{record.status}</Badge>
                    </td>
                    <td className="px-6 py-4 text-right font-medium text-slate-900 whitespace-nowrap">${record.amount.toFixed(2)}</td>
                    <td className="px-6 py-4 text-center whitespace-nowrap">
                      {record.status !== PaymentStatus.Paid && (
                        <Button variant="ghost" className="h-8 px-2 text-green-600 hover:text-green-700 hover:bg-green-50" onClick={() => setPaymentModalOpen(record.id)} title="Mark as Paid">
