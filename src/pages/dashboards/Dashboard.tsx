...
export const Dashboard = () => {
  ...
  const { clients, conflicts, todaySessions, weeklySessions, newLeads, activeClients, weeklyScheduleData, clientsToWatch, toggleSessionComplete } = useDashboard()
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          title={ts('sessionsToday')}
          value={todaySessions.length.toString()}
          icon={Calendar}
          description={`${todaySessions.filter((s) => s.completed).length} ${ts('completed').toLowerCase()}`}
        />
        <StatCard
          title={ts('thisWeeksSessions')}
          value={weeklySessions.length.toString()}
          icon={Activity}
          description={`${weeklySessions.filter((s) => !s.completed).length} ${ts('pending').toLowerCase()}`}
        />
