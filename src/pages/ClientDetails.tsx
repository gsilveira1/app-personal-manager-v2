...
export const ClientDetails = () => {
  ...
  const handleSaveMedicalHistory = () => { updateClient(client.id, { medicalHistory: medicalHistoryBuffer }); setIsEditingMedicalHistory(false); };
  const handleStartEditMedicalHistory = () => { setMedicalHistoryBuffer(client.medicalHistory || {}); setIsEditingMedicalHistory(true); };
        ...
        <div className="space-y-6">
          <Card className="p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {!isEditingMedicalHistory ? ( <button onClick={handleStartEditMedicalHistory} className="text-slate-400 hover:text-indigo-600"><Edit2 className="h-4 w-4" /></button> ) : ( <button onClick={handleSaveMedicalHistory} className="text-green-600 hover:text-green-700"><Save className="h-4 w-4" /></button> )}
            </div>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="font-bold text-slate-900">
                        ...
                        <button onClick={(e) => {e.stopPropagation(); setIsEditEvalModalOpen(true)}} className="text-slate-400 hover:text-indigo-600"><Edit2 className="h-4 w-4" /></button>
                        <button onClick={(e) => {e.stopPropagation(); setIsRemoveEvalModalOpen(true)}} className="text-slate-400 hover:text-indigo-600"><Trash2 className="h-4 w-4" /></button>
                      </div>
...
const 
EvaluationModal = ({ clientId, onClose, onSave, initialData }: { clientId: string; onClose: () => void; onSave: (e: Omit<Evaluation, 'id'>) => void; initialData?: Evaluation | null }) => {
