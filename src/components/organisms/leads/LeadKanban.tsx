      ...
      {stages.map((stage) => (
        <div key={stage.id} data-testid={`lead-column-${stage.id}`} className="flex flex-col gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="ml-auto text-xs font-semibold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">{byStage[stage.id].length}</span>
          </div>

          {/* Cards */}
          <div className="flex flex-col gap-3 min-h-[120px]">
            {byStage[stage.id].map((lead) => (
              <LeadCard key={lead.id} client={lead} stage={stage.id} onClick={() => onLeadClick(lead)} />
            ))}
            {byStage[stage.id].length === 0 && (
              <div className="flex items-center justify-center h-16 border-2 border-dashed border-slate-200 rounded-xl text-slate-300 text-sm">{t('noLeadsInColumn')}</div>
            )}
          </div>
        </div>
      ))}
