...
const WeekView = ({ date, sessions, clients, onSessionClick, onToggleComplete, onAreaClick, dragHandlers, blocks = [], onBlockClick }: any) => {
  ...
  return (
    <div data-testid="week-view" className="space-y-6">
      {weekDays.map((day) => {
          ...
          >
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 space-y-3 pt-1">
                {(() => {
                  const daySessions = sessions.filter((s: Session) => isSameDay(parseISO(s.date), day))
                  const dayBlocks = (blocks as MaterializedBlock[]).filter((b) => isSameDay(parseISO(b.start), day))

                  if (daySessions.length === 0 && dayBlocks.length === 0) {
                    return (
                      <div
                  ...
                  return (
                    <>
                      {dayBlocks
                        .sort((a: MaterializedBlock, b: MaterializedBlock) => new Date(a.start).getTime() - new Date(b.start).getTime())
                        .map((block: MaterializedBlock) => (
                          <BlockCard key={block.id} block={block} onClick={() => onBlockClick?.(block)} />
                        ))}
                      {daySessions
