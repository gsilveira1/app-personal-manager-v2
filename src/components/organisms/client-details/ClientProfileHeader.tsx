...
export const ClientProfileHeader: React.FC<ClientProfileHeaderProps> = ({ client, clientPlan, isUploadingAvatar, avatarInputRef, onAvatarChange }) => {
  ...
  return (
    <>
            ...
            {clientPlan && (
              <div className="flex flex-col items-start md:items-end gap-1 shrink-0">
                ...
                <span className="text-xs text-slate-500">
                  ...
                  {clientPlan.durationMinutes ? ` · ${clientPlan.durationMinutes}min` : ''} · R$ {clientPlan.price.toFixed(2)}
                  {tco('perMonth')}
                </span>
              </div>
            )}
      ...
      <ClientProfileEditorModal isOpen={isEditorOpen} onClose={() => setIsEditorOpen(false)} client={client} />
    </>
  )
}
