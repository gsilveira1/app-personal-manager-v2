...
interface WorkoutEditorModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (workout: Omit<WorkoutPlan, 'id' | 'createdAt'>) => void
...
export const WorkoutEditorModal = ({ isOpen, onClose, onSave, initialData, client }: WorkoutEditorModalProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const workout: Omit<WorkoutPlan, 'id' | 'createdAt'> = {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {exercises.map((exercise, index) => (
                <div
                  key={index}
                  className={`flex flex-col sm:flex-row gap-3 items-start p-4 rounded-lg border transition-all hover:border-indigo-300 ${exercise.isWarmup ? 'bg-orange-50 border-orange-200' : 'bg-slate-50 border-slate-200'}`}
                >
                  <div className="flex sm:flex-col gap-1 sm:mt-6 mr-1">
                    <button
                      type="button"
                  ...
                  <div className="flex-1 w-full grid gap-3">
                    <div className="flex items-center justify-between">
                      <label
                        className={`flex items-center gap-2 cursor-pointer text-xs font-semibold px-2 py-1 rounded border transition-colors ${exercise.isWarmup ? 'bg-orange-100 text-orange-700 border-orange-200' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}
                      >
                        <input
                          type="checkbox"
                          checked={!!exercise.isWarmup}
                          onChange={(e) => handleExerciseChange(index, 'isWarmup', e.target.checked)}
